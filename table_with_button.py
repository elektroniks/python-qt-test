import sys
from PyQt5.QtCore import Qt, QModelIndex, QAbstractTableModel, QItemSelectionModel, QRectF, QEvent
from PyQt5.QtGui import QStandardItemModel, QPainter, QMouseEvent, QPalette
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableView, QPushButton, QVBoxLayout, QWidget, QStyledItemDelegate, QStyle
...
class ButtonDelegate(QStyledItemDelegate):
    ...
    def drawButton(self, painter, option, index):
        if option.state & QStyle.State_MouseOver:
            color = option.palette.color(QPalette.Button).darker(110)
        else:
            color = option.palette.color(QPalette.Button)

        button_rect = option.rect.adjusted(4, 4, -4, -4)
        self.button_rects[index] = button_rect
        ...
        painter.drawText(button_rect, Qt.AlignCenter, "Delete")
    ...
    def editorEvent(self, event, model, option, index):
        if index.column() == index.model().columnCount() - 1:
            if event.type() == QEvent.MouseButtonRelease and event.button() == Qt.LeftButton:
                button_rect = self.button_rects.get(index)
